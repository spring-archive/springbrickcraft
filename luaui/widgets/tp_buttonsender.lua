...
function widget:KeyRelease(key)
	--Spring.Echo ("key=" .. key)
	if (key == KP8) then 
		Spring.Echo ("und zack")
		Spring.SendLuaRulesMsg ("tpkey|8")
	end
	if (key == KP2) then
		--Spring.Echo ("shrink")
		Spring.SendLuaRulesMsg ("tpkey|2")
	end
	if (key == KP4) then 
		--Spring.Echo ("narrow")
		Spring.SendLuaRulesMsg ("tpkey|4")
	end
	if (key == KP6) then 
		--Spring.Echo ("wider")
		Spring.SendLuaRulesMsg ("tpkey|6")
	end
	if (key == KP7) then 
		--Spring.Echo ("shorter")
		Spring.SendLuaRulesMsg ("tpkey|7")
	end
	if (key == KP9) then
		--Spring.Echo ("longer")
		Spring.SendLuaRulesMsg ("tpkey|9")
	end
	if (key == W) then
		--Spring.Echo ("to front")
		Spring.SendLuaRulesMsg ("tpkey|W")
	end
	if (key == S) then
		--Spring.Echo ("to back")
		Spring.SendLuaRulesMsg ("tpkey|S")
	end
	if (key == A) then
		--Spring.Echo ("to left")
		Spring.SendLuaRulesMsg ("tpkey|A")
	end
	if (key == D) then
		--Spring.Echo ("to right")
		Spring.SendLuaRulesMsg ("tpkey|D")
	end

	if (key == E) then
		--Spring.Echo ("to up")
		Spring.SendLuaRulesMsg ("tpkey|E")
	end
	if (key == Q) then
		--Spring.Echo ("to lower")
		Spring.SendLuaRulesMsg ("tpkey|Q")
	end
	if (key == KP5) then
		--Spring.Echo ("changing d!")
		Spring.SendLuaRulesMsg ("tpkey|5")
	end
end
