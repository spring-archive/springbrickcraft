...
do
  ...
  function JustGetOverlayInfos(unitID,unitDefID, ud)
    ...
    --// PARALYZE
	  if (emp>0.01)and(hp>0.01)and((not morph) or morph.combatMorph)and(emp<90000) then -- emp<1e8
...
--no blue flashing units when aircraft is crashing
--[[
      --// PARALYZE
...
--]]
	  ...
	  --added by knorke   AddBar(title,progress,color_index,text,color)
	  --// MANA
      local manaState = GetUnitRulesParam(unitID,"manaPercent")
      if (manaState) then
        AddBar("Energy",manaState/100,"mana","")		
      end
