-- *** how to stop engine trees from spawning? --wat?
...
--the mapconfig file returns:
--featuresTable, removeFeatures true/false
...
--local mapconfig_fn = Game.mapName .. "_BRICKCRAFT_features.lua"		--name of map config files
local mapconfig_fn = Game.mapName .. "\\BRICKCRAFT_features.lua"		--name of map config files
if (VFS.FileExists(mapconfig_fn)) then 
	Spring.Echo ("tp_feature_spawn.lua: found" .. mapconfig_fn .." in mod root folder or map")
	mapfeatures, removeFeatures = VFS.Include(mapconfig_fn)
else
	if (VFS.FileExists("mapconfigs\\" .. mapconfig_fn)) then 
		...
		mapfeatures, removeFeatures = VFS.Include("mapconfigs\\" .. mapconfig_fn)
	else
		Spring.Echo ("tp_feature_spawn.lua:  " .. mapconfig_fn .. " NOT FOUND AT ALL.")
	end
end
...
local function PutFeaturesOnMap ()
	...
	if (mapfeatures) then
		...
		for i in pairs(mapfeatures) do			
			...
			local fid = Spring.CreateFeature (mapfeatures[i].name,  mapfeatures[i].x, y, mapfeatures[i].z, mapfeatures[i].rot or math.random (0,32000), GetGaiaTeamID)			
			if (not fid) then Spring.Echo ("feature_spawn: CreateFeature failed") end
		end
	end
end
