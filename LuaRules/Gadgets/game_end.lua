...
function gadget:GetInfo()
	return {
		...
		author    = "Andrea Piras, hacklol fix by knorke",
		date      = "August, 2010",
...
local teamDeathMode = "allyzerounits" --modOptions.teamdeathmode or "teamzerounits"
-- sharedDynamicAllianceVictory is a C-like bool
local sharedDynamicAllianceVictory = 0 --tonumber(modOptions.shareddynamicalliancevictory) or 0
-- ignoreGaia is a C-like bool
local ignoreGaia = 1 --tonumber(modOptions.ignoregaiawinner) or 1
...
local function KillAllyTeamsZeroUnits()
	...
	for allyTeamID, unitCount in pairs(allyTeamUnitCount) do
		--Spring.Echo ("allyTeamID:"..allyTeamID .. "   unitCount:"..unitCount)
		if unitCount <= 0 then
...
function gadget:GameFrame(frame)
	--if (frame==30) then Initialize() end
	if (frame > 30 and (frame%64) == 0) then -- only do a check very rarely
		CheckGameOver()
...
function gadget:Initialize()
	...
	for _,allyTeamID in ipairs(allyTeams) do
		...
		gadget:UnitCreated(0, 0, teamID)  --hack lol***
